...
float effectiveTargetAngle = 0.0;
...
void loop(){
  ...
  pulse_duration2 = pulseIn(receiver_pin2, HIGH);

  // // For the right throttle, flip the mapping so 999 -> 0.5 and 2000 -> -0.5
  // float throttleRight = 5.0 - (((float)pulse_duration - 999.0) * (10.0 / (2000.0 - 999.0)));
  float throttleRight = 0.5 - ((float)(pulse_duration - 999) / 1001.0);
  // Quantize to increments of 0.005.
  throttleRight = round(throttleRight / 0.001) * 0.001;

  
   float throttleLeft = ((float)pulse_duration2 - 999.0) * (4.0 / 1001.0) - 2.0;
  // Quantize to increments of 0.001:
  throttleLeft = round(throttleLeft / 0.001) * 0.001;
  ...
  if (mpu.dmpGetCurrentFIFOPacket(FIFOBuffer)) { 
    ...
    effectiveTargetAngle = targetAngle + driveOffset;
  ...
  if (!wait_time){
    ...
    odrv0.setVelocity(-controlOutput + turn_command, 60.0f);
    odrv1.setVelocity(controlOutput + turn_command, 60.0f);
    previousError = currentError;
    lastTime = currentTime;

driveOffset = throttleRight;
turn_command = throttleLeft;
...
if (Serial.available() > 0) {
    ...
    //     int command = input.substring(1).toInt();
    ...
    //     //driveOffset = 0.7 * driveOffset + (1.0 - 0.7) * driveOffset;
    //     Serial.print("Drive Offset updated to: ");
    //     Serial.println(driveOffset);
    // } 
    //   if (input.startsWith("t")) {
    //     // Convert command value (e.g., 1 for forward, -1 for reverse)
    //     int turn = input.substring(1).toInt();
    //     // Set drive offset angle (tweak the multiplier to adjust sensitivity)
    //     turn_command = turn * 0.1; // 5 degrees tilt for moderate speed
    //     //turn_command = 0.7 * turn_command + (1.0 - 0.7) * turn_command;
    //     Serial.print("Drive turning updated to: ");
...
//controller();
  ...
  Serial.print("Right Throttle (raw): ");
  Serial.print(pulse_duration);
  Serial.print(" -> Mapped: ");
  Serial.println(throttleRight, 1); // 1 decimal place

  Serial.print("Left Throttle (raw): ");
  Serial.print(pulse_duration2);
  Serial.print(" -> Mapped: ");
  Serial.println(throttleLeft, 1);
    ...
    Serial.print("effectiveTargetAngle: ");
    Serial.println(effectiveTargetAngle);
    Serial.print("turn_command: ");
    Serial.println(turn_command);
    Serial.print("drive_offset: ");
    Serial.println(driveOffset);
...
void controller() {
  ...
  // For the right throttle, flip the mapping so 999 -> 5 and 2000 -> -5
  float throttleRight = 5.0 - (((float)pulse_duration - 999.0) * (10.0 / (2000.0 - 999.0)));
  
  // For the left throttle, mapping remains unchanged: 999 -> -5 and 2000 -> 5
  float throttleLeft  = ((float)pulse_duration2 - 999.0) * (10.0 / (2000.0 - 999.0)) - 5.0;
